...
use vars
  qw($currentTopic $currentWeb %seenAnchorNames $makeIndexCounter %filteredTopic);
use POSIX qw(ceil);
use Foswiki::Func();
...
sub handleFilter {
    ...
    if ( defined $theText ) {    # direct text
        ...
        if ( $text =~ /%STARTINCLUDE%(.*)%STOPINCLUDE%/gs ) {
            ...
            if ( $theExpand eq 'on' ) {
                $text =
                  Foswiki::Func::expandCommonVariables( $text, $currentTopic,
                    $currentWeb );
...
sub handleMakeIndex {
    ...
    # compute the list
    $theList =
      Foswiki::Func::expandCommonVariables( $theList, $theTopic, $theWeb )
...
#writeDebug("group=$descriptor{group}, item=$descriptor{item} crit=$descriptor{crit}");
    ...
    # sort it
    @theList = sort { $a->{crit} cmp $b->{crit} } @theList
      if $theSort =~ /nocase|on/;
    @theList = sort { $a->{crit} <=> $b->{crit} } @theList if $theSort eq 'num';
    @theList = reverse @theList if $theReverse eq 'on';

    my $result =
      "<table class='fltLayoutTable' cellspacing='0' cellpadding='0'>\n<tr>\n";
...
sub handleFormatList {
    ...
    if ($theTokenize) {
        $theList =~
s/($theTokenize)/$tokenNr++; $tokens{'token_'.$tokenNr} = $1; 'token_'.$tokenNr/gems;
    }
